...
class Card {
  ...
  class Player {
    ...
    // dealCards is a method of the player class, in order to use it it always needs to be attached to a player and called
    dealCards(halfOfDeck){
      for (let i = 0; i < 26; i++) {
        // this.hand will represent the player when instantiated
  ...
  function playGame() {
    let deck = new Deck();
    let player1 = new Player('Person 1');
    let player2 = new Player('Person 2');
    ...
    for (let i = 0; i < player1.hand.length; i++) {
      ...
      if (player1.hand[i].value < player2.hand[i].value) {
        ...
        console.log(`${player1.hand[i].cardNum} of ${player1.hand[i].suit} = ${player2.hand[i].cardNum} of ${player2.hand[i].suit}, its a tie, no one gets a point`);
      }
    }
